...
from optims_sched.muon_revised import SingleDeviceMuonWithAuxAdam
from optims_sched.lr_sign10_rsclF import SingleDeviceSign10RsclFWithAuxAdam
from optims_sched.auto_cos_inc_revised import SingleDeviceAutoCosIncWithAuxAdam
from optims_sched.rank_wsd_scheduler import RankAwareWarmupStableLinearScheduler
#//=================================
...
def init_results_log_optim(results_path, args, optimizer, start):
    ...
    with open(results_path, 'a') as f:
        ...
        if getattr(args, 'train_scheduler', 'rank_wsd') in {'exp_decay', 'original'}:
            ...
            if getattr(args, 'train_scheduler', 'rank_wsd') in {'rank_wsd', 'wsd'}:
...
def append_results_log_optim(results_path, i, global_step, loss, psnr, optimizer):
    ...
    rank_saturated = optimizer.param_groups[0].get('rank_saturated', None)
    rank_role = optimizer.param_groups[0].get('rank_wsd_role', None)
    rank_log = ""
    ...
    if rank_role is not None:
        rank_log += f" rank_role={rank_role}"
    if rank_saturated is not None:
        rank_log += f" rank_saturated={rank_saturated}"
    with open(results_path, 'a') as f:
        f.write(
            f"iter={i} global_step={global_step} "
            f"loss={loss.item():.10f} psnr={psnr.item():.6f} "
...
def _optimizer_supports_progressive_rank(optimizer_name):
    # Only our auto-cos increasing-rank optimizer should receive late decay by rank saturation.
    # Muon and fixed-rank optimizers are already saturated and are handled as early-decay cases.
    return optimizer_name in {'aux-sign-auto-cos-inc'}
...
def _resolve_decay_start_step(args, total_iters, warmup_steps, rank_schedule_steps=None):
    """Return only user-explicit decay starts.

    Rank-aware WSD now tracks optimizer.param_groups at runtime, so the default
    progressive-rank decay start is not hard-coded here. If the user passes
    --sched_decay_start_step or --sched_decay_start_frac, that explicit value is
    respected. Otherwise None lets RankWSD-Auto infer the decay start from rank
    metadata.
    """
    total_iters = int(total_iters)
    ...
    if explicit_steps > 0:
        return min(total_iters - 1, max(warmup_steps, explicit_steps))

    decay_frac = float(getattr(args, 'sched_decay_start_frac', 0.0) or 0.0)
    if decay_frac > 0.0:
        resolved = int(round(total_iters * decay_frac))
        return min(total_iters - 1, max(warmup_steps, resolved))
...
def _sync_optimizer_schedule_config(optimizer, args, reset_rank_fields=False):
    ...
    for group in optimizer.param_groups:
        ...
        if args.optimizer in {'aux-muon', 'muon'}:
            # Muon is treated as full-rank from initialization. The revised muon.py
            # also writes this metadata internally, but keeping it here makes old
            # checkpoints/config reloads safer.
            group['ns_steps'] = int(resolved_ns_steps)
            full_rank_values = []
            for p in group.get('params', []):
                if p.ndim >= 2:
                    full_rank_values.append(int(min(p.shape[0], p.reshape(p.shape[0], -1).shape[1])))
            full_rank = max(full_rank_values) if full_rank_values else 1
            ...
            group['current_rank'] = int(full_rank)
            group['current_target_rank'] = int(full_rank)
            group['rank_start'] = int(full_rank)
            group['rank_end'] = int(full_rank)
            group['rank_saturated'] = True
            group['rank_wsd_decay_ready'] = True
            group.setdefault('rank_saturation_step', 0)
        ...
        if args.optimizer == 'aux-sign-auto-cos-inc':
            ...
            group['rank_wsd_role'] = 'progressive_lowrank'
            group['is_full_rank'] = False
            group['is_progressive_rank'] = True
            group['rank_schedule_type'] = 'cosine_increase'
            group['rank_saturated'] = int(group.get('current_rank', args.lowrank_rank_start)) >= int(group['current_target_rank'])
            group['rank_wsd_decay_ready'] = bool(group['rank_saturated'])
            group.setdefault('rank_saturation_step', 0 if group['rank_saturated'] else None)
        ...
        if args.optimizer == 'aux-sign10-rsclF':
            # Fixed low-rank baseline. It is not progressive, so RankWSD-Auto should
            # not delay decay until late training.
            fixed_rank = int(getattr(args, '_resolved_fixed_rank', _resolve_fixed_rank(args)))
            group['rank'] = fixed_rank
            group['rank_start'] = fixed_rank
            group['rank_end'] = fixed_rank
            group['current_rank'] = fixed_rank
            group['current_target_rank'] = fixed_rank
            group['rank_wsd_role'] = 'fixed_lowrank'
            group['is_full_rank'] = False
            group['is_progressive_rank'] = False
            group['rank_schedule_type'] = 'fixed_lowrank'
            group['rank_saturated'] = True
            group['rank_wsd_decay_ready'] = True
            group.setdefault('rank_saturation_step', 0)
            group['oversample'] = int(args.lowrank_oversample)
...
def _build_lr_scheduler(args):
    if args.train_scheduler in {'rank_wsd', 'wsd'}:
        return RankAwareWarmupStableLinearScheduler(
            ...
            default_decay_start_frac=args.sched_default_decay_start_frac,
            default_decay_end_frac=args.rank_wsd_default_decay_end_frac,
            fullrank_decay_start_frac=args.rank_wsd_fullrank_decay_start_frac,
            fullrank_decay_end_frac=args.rank_wsd_fullrank_decay_end_frac,
            rank_saturation_patience_steps=args.rank_wsd_saturation_patience_steps,
        )

    if args.train_scheduler in {'exp_decay', 'original'}:
    ...
    raise ValueError(
        ...
        "Supported options are: 'rank_wsd'/'wsd', 'exp_decay'/'original'."
    )




def _step_train_scheduler(train_scheduler, global_step, optimizer):
    """Call scheduler.step with optimizer only for RankWSD-Auto.

    Original NeRF exponential decay and warmup-cosine schedulers keep the old
    step(global_step) interface. RankWSD-Auto needs optimizer metadata because
    it reads current_rank/current_target_rank from optimizer.param_groups.
    """
    if isinstance(train_scheduler, RankAwareWarmupStableLinearScheduler):
        return train_scheduler.step(global_step, optimizer=optimizer)
    return train_scheduler.step(global_step)
...
def _step_scheduler_with_optional_optimizer(train_scheduler, global_step, optimizer):
    """Call rank-aware schedulers with optimizer metadata and legacy schedulers without it."""
    try:
        return train_scheduler.step(global_step, optimizer=optimizer)
    except TypeError as exc:
        # OriginalExpDecayScheduler and external WarmupCosineScheduler usually expose
        # step(global_step) only. Avoid hiding unrelated TypeErrors from scheduler internals.
        if "optimizer" not in str(exc):
            raise
        return train_scheduler.step(global_step)
...
def create_nerf(args):
    ...
    if args.optimizer in ('ori-adam', 'adam'):
        ...
        aux_param_groups[0].update(
            dict(
                rank=fixed_rank,
                rank_start=fixed_rank,
                rank_end=fixed_rank,
                warmup_steps=1,
                oversample=args.lowrank_oversample,
                ns_steps=int(getattr(args, '_resolved_lowrank_ns_steps', _resolve_ns_steps(args))),
                lowrank_rescale=bool(getattr(args, '_resolved_lowrank_rescale', _resolve_lowrank_rescale(args))),
                auto_init_rank_start=False,
                init_probe_steps=1,
                init_energy=args.lowrank_init_energy,
                init_round_multiple=args.lowrank_init_round_multiple,
                rank_wsd_role='fixed_lowrank',
                is_progressive_rank=False,
                current_rank=fixed_rank,
                current_target_rank=fixed_rank,
                rank_saturated=True,
            )
        )
...
def config_parser():
    ...
    parser.add_argument("--train_scheduler", "--train-scheduler", dest="train_scheduler", type=str, default='rank_wsd',
                        choices=['rank_wsd', 'wsd', 'exp_decay', 'original'],
                        help=("LR scheduler family. 'rank_wsd'/'wsd' = rank-tracking WSD; "
                              "'exp_decay'/'original' = original NeRF exponential LR decay using --lrate_decay."))
    ...
    parser.add_argument("--rank_wsd_default_decay_end_frac", type=float, default=1.0,
                        help="Fallback decay-end fraction for Adam/unknown optimizers under rank_wsd.")
    parser.add_argument("--rank_wsd_fullrank_decay_start_frac", type=float, default=0.2,
                        help="For Muon/fixed-rank optimizers under rank_wsd, start decay at this fraction because rank is already saturated.")
    parser.add_argument("--rank_wsd_fullrank_decay_end_frac", type=float, default=0.6,
                        help="For Muon/fixed-rank optimizers under rank_wsd, reach min LR at this fraction, then hold min LR. This implements fast decay.")
    parser.add_argument("--rank_wsd_saturation_patience_steps", type=int, default=0,
                        help="Optional delay after progressive rank saturation before WSD decay starts.")
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        sched = _step_train_scheduler(train_scheduler, global_step, optimizer)
        ...
        current_rank = sched.get("rank_current", None)
        target_rank = sched.get("rank_target", None)
        decay_start_step = sched.get("decay_start_step", None)
        decay_end_step = sched.get("decay_end_step", None)
        decay_start_source = sched.get("decay_start_source", None)
        decay_end_source = sched.get("decay_end_source", None)
        ...
        if i % args.i_print==0:
            rank_msg = ""
            if current_rank is not None or target_rank is not None:
                rank_msg = f" rank={current_rank}/{target_rank} decay_start={decay_start_step} decay_source={decay_start_source}"
            tqdm.write(
...
"""
CUDA_VISIBLE_DEVICES=0 python 99_run_nerf.py --config configs/lego.txt --basedir logs/rankwsd_auto --expname lego_ours_rankwsd_auto --optimizer aux-sign-auto-cos-inc --train-scheduler rank_wsd --muon_lrate 3e-3 --lrate 5e-4 --lowrank_rank_start 150 --lowrank_rank_end 250 --lowrank_schedule_steps 100000 --lowrank_auto_init_rank_start --N_iters 200000
