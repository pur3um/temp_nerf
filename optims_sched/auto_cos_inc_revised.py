# auto_cos_inc_rank_revised.py
# Drop-in replacement for optims/auto_cos_inc_rank.py with rank metadata for RankWSD-Auto.
...
def _attach_progressive_rank_metadata(group):
    target = max(int(group.get("rank", 1)), int(group.get("rank_end", group.get("rank", 1))))
    current = int(group.get("current_rank", group.get("rank_start", group.get("rank", 1))))
    saturated = current >= target
    start = int(group.get("rank_start", group.get("rank", current)))
    denom = max(1, target - start)
    progress = max(0.0, min(1.0, (current - start) / float(denom)))
    ...
    group["current_target_rank"] = target
    group["current_rank"] = current
    group["rank_saturated"] = bool(saturated)
    group["rank_progress"] = float(progress)
    group["rank_wsd_decay_ready"] = bool(saturated)
    group.setdefault("rank_saturation_step", 0 if saturated else None)
    return group
...
class SingleDeviceAutoCosIncWithAuxAdam(torch.optim.Optimizer):
    """
    Progressive low-rank Muon/sign optimizer + auxiliary Adam.

    This version writes rank metadata into param_groups so RankWSD-Auto can
    determine decay_start_step by reading current_rank and current_target_rank.
    """
    ...
    def __init__(self, param_groups):
        ...
        for group in param_groups:
            ...
            if g["use_muon"]:
                ...
                # Auto-initialize rank_start from early sketched Frobenius-energy curve.
                ...
                _attach_progressive_rank_metadata(g)
            else:
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            if group["use_muon"]:
                ...
                # (1) Optional auto rank_start estimation.
                ...
                target_rank = max(int(group["rank"]), int(group["rank_end"]))
                saturated = applied_rank >= target_rank
                progress_start = int(group.get("rank_start", group.get("rank", applied_rank)))
                progress_denom = max(1, int(target_rank) - progress_start)
                progress = max(0.0, min(1.0, (int(applied_rank) - progress_start) / float(progress_denom)))

                group["current_rank"] = int(applied_rank)
                group["current_target_rank"] = int(target_rank)
                group["rank_saturated"] = bool(saturated)
                group["rank_progress"] = float(progress)
                group["rank_wsd_decay_ready"] = bool(saturated)
                if saturated and group.get("rank_saturation_step") is None:
                    group["rank_saturation_step"] = int(group_step)
                group["current_method"] = (
                    "cosine_inc_auto_start" if bool(group.get("auto_init_rank_start", False))
                    else "cosine_inc_closed_form"
                )
                _attach_progressive_rank_metadata(group)
