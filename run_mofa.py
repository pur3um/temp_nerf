...
from optims.mofasgd import MomentumFactorizedSGD
...
def init_results_log_mofa(results_path, args, optimizer_mofa, optimizer_adam, start):
    mofa_tensor_count = sum(len(group['params']) for group in optimizer_mofa.param_groups)
    adam_tensor_count = sum(len(group['params']) for group in optimizer_adam.param_groups)
    mofa_total_params = sum(p.numel() for group in optimizer_mofa.param_groups for p in group['params'])
    ...
    with open(results_path, 'a') as f:
        ...
        f.write("script: run_nerf_mofasgd.py\n")
        f.write(f"expname: {args.expname}\n")
        ...
        f.write("  optimizer_main: MoFaSGD\n")
        ...
        f.write(f"  mofa_eps: {args.mofa_eps}\n")
        ...
        f.write(f"  mofa_nesterov: {args.mofa_nesterov}\n")
        ...
        f.write(f"  mofa_tensor_count: {mofa_tensor_count}\n")
        f.write(f"  adam_tensor_count: {adam_tensor_count}\n")
        f.write(f"  mofa_total_params: {mofa_total_params}\n")
        f.write(f"  adam_total_params: {adam_total_params}\n")
        f.write("train_psnr_log:\n")


def append_results_log_mofa(results_path, i, global_step, loss, psnr, optimizer_mofa, optimizer_adam):
    mofa_lr = optimizer_mofa.param_groups[0]['lr']
    adam_lr = optimizer_adam.param_groups[0]['lr']
    with open(results_path, 'a') as f:
        f.write(
            f"iter={i} global_step={global_step} "
            f"loss={loss.item():.10f} psnr={psnr.item():.6f} "
            f"mofa_lr={mofa_lr:.8e} adam_lr={adam_lr:.8e}\n"
        )
...
def split_nerf_params(net):
    ...
    for name, p in net.named_parameters():
        ...
        # bias / norm-like / scalar-vector params -> Adam
        if p.ndim < 2:
            adam_params.append(p)
            continue

        # keep final output heads on Adam
        if (
        ...
        # remaining 2D hidden weights -> MoFaSGD
        mofa_params.append(p)

    return mofa_params, adam_params
...
def create_nerf(args):
    ...
    mofa_params, adam_params = split_nerf_params(model)
    ...
    if args.N_importance > 0:
        model_fine = NeRF(D=args.netdepth_fine, W=args.netwidth_fine,
                          input_ch=input_ch, output_ch=output_ch, skips=skips,
                          input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs).to(device)
        mofa_fine_params, adam_fine_params = split_nerf_params(model_fine)
        mofa_params += mofa_fine_params
        adam_params += adam_fine_params

    grad_vars = mofa_params + adam_params

    print(f'Param split: MoFaSGD={len(mofa_params)} tensors, Adam={len(adam_params)} tensors')
    ...
    # Create optimizers
    optimizer_mofa = MomentumFactorizedSGD(
        params=list(mofa_params),
        lr=args.mofa_lrate,
        rank=args.mofa_rank,
        beta=args.mofa_beta,
        eta1=args.mofa_eta1,
    ...
    optimizer_adam = torch.optim.Adam(params=list(adam_params), lr=args.lrate, betas=(0.9, 0.999))
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        if 'optimizer_mofa_state_dict' in ckpt and 'optimizer_adam_state_dict' in ckpt:
            optimizer_mofa.load_state_dict(ckpt['optimizer_mofa_state_dict'])
            optimizer_adam.load_state_dict(ckpt['optimizer_adam_state_dict'])
        elif 'optimizer_state_dict' in ckpt:
            print('Old or incompatible checkpoint format detected: skipping optimizer state reload for split MoFaSGD/Adam optimizers.')
    ...
    return render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer_mofa, optimizer_adam
...
def config_parser():
    ...
    parser.add_argument("--i_img",     type=int, default=10000000000, 
    ...
    # MoFaSGD optimizer options
    parser.add_argument("--mofa_lrate", type=float, default=5e-4,
                        help="learning rate for MoFaSGD 2D weight branch")
    parser.add_argument("--mofa_rank", type=int, default=16,
                        help="target rank for factorized momentum; clipped per tensor when needed")
    parser.add_argument("--mofa_beta", type=float, default=0.9,
                        help="momentum decay for MoFaSGD")
    parser.add_argument("--mofa_decay", type=float, default=0.0,
                        help="AdamW-style weight decay for MoFaSGD branch")
    parser.add_argument("--mofa_eta1", type=float, default=1.0,
                        help="scale for low-rank MoFaSGD update")
    parser.add_argument("--mofa_eta2", type=float, default=0.0,
                        help="scale for orthogonal complement gradient update")
    parser.add_argument("--mofa_eps", type=float, default=1e-4,
                        help="epsilon for singular-value masking")
    parser.add_argument("--mofa_max_value", type=float, default=10000.0,
                        help="clip value for reciprocal singular values")
    parser.add_argument("--mofa_use_current_projection", action="store_true",
                        help="use updated U/V when computing eta2 orthogonal complement")
    parser.add_argument("--mofa_use_reciprocal_s", action="store_true",
                        help="use U diag(1/S) V^T instead of the default U V^T update")
    parser.add_argument("--mofa_nesterov", action="store_true",
                        help="apply the optional second momentum-factor update")
...
def train():
    ...
    # Create nerf model
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer_mofa, optimizer_adam = create_nerf(args)
    global_step = start

    results_path = os.path.join(basedir, expname, 'results.txt')
    init_results_log_mofa(results_path, args, optimizer_mofa, optimizer_adam, start)
    ...
    for i in trange(start, N_iters):
        ...
        new_mofa_lrate = args.mofa_lrate * (decay_rate ** (global_step / decay_steps))
        new_adam_lrate = args.lrate * (decay_rate ** (global_step / decay_steps))

        for param_group in optimizer_mofa.param_groups:
            param_group['lr'] = new_mofa_lrate
        ...
        if i%args.i_weights==0:
            ...
            torch.save({
                ...
                'optimizer_mofa_state_dict': optimizer_mofa.state_dict(),
        ...
        if i%args.i_print==0:
            ...
            append_results_log_mofa(results_path, i, global_step, loss, psnr, optimizer_mofa, optimizer_adam)
